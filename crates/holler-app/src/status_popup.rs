...
fn paint(
    ...
    // (make_pill_window) so there's no dark backdrop behind the corners. The SDF
    // still paints the subtle AA edge ring and is a graceful same-colour
    // fallback if the layer clip isn't available.
    for y in 0..h {
        for x in 0..w {
